...
rclone config
RcloneBrowser /kapitainsky/RcloneBrowser/releases
```
- 挂载
```
cd /d d:\rclone
...
rclone mount --cache-dir C:\rclone\Cache --vfs-cache-mode full --buffer-size 64M --low-level-retries 200 --dir-cache-time 12h --vfs-read-chunk-size 64M --vfs-read-chunk-size-limit 1G 
```
